...
def main_dmc(game_name, level, time_flag):
    ...
    def dmc_maze(game_name, level, time_flag, q1, q2):
        ...
        agent = DMC(game_name, map_id, time_flag, 0).cuda(device='cuda:%d'%(0))
        episode_num = 0
        sumt = 0
        while (1):
            episode_num += 1
            ...
            if time_flag in [0, 7]:
                    h_i = torch.autograd.Variable(to_tensor(np.zeros((1,100)))).cuda(device=cud)
                    c_i = torch.autograd.Variable(to_tensor(np.zeros((1,100)))).cuda(device=cud)
            if time_flag == 1:
            ...
            if time_flag in [2, 3, 4, 5]:
                    ...
                    c_i = torch.autograd.Variable(to_tensor(np.zeros((1, 100)))).cuda(device=cud)
            if time_flag == 6:
                    _map = torch.autograd.Variable(to_tensor(np.zeros((1, 20)))).cuda(device=cud)
            ...
            for t in range(30):
                if 1:
                    if time_flag == 0:
                        s_moved = torch.autograd.Variable(torch.from_numpy(np.array([[s]])).float()).cuda(device=cud)
    ...
    cuda_device = torch.device('cuda:%d'%(0))
    cud = torch.device('cuda:%d'%(0))
    ...
    for j in range(len(p)):
        p[j].start()
    map_id = level - 1
    model = DMC(game_name, map_id, time_flag, 0).cuda(device=cuda_device)
...
def allocate(game_name='Maze', level=1, method='t-hci'):
    time_flag = ['lstm', '', '', 't-hci', 'sai', 'wmg', 'nm', 'gru', 'gate'].index(method)
    if game_name == 'babyAI':
            main_ac(game_name, level, time_flag)
    if game_name == 'Maze':
            main_dmc(game_name, level, time_flag)
