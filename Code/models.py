...
class DMC(nn.Module):
    ...
    def forward_pure_pr(self, x, h, c):
        x_pr = F.relu(self.wta_pr_fc1(x))
        h, c = self.wta_lstm(x_pr, (h, c))
        x_pr = F.relu(self.attention_pr_fc2(h))
        out_pr = self.attention_pr_out(x_pr)
        return out_pr, h, c
    ...
    def choose_action_markov(self, x, epsilon):
        x = copy.deepcopy(x)
        state_moved = torch.autograd.Variable(torch.from_numpy(np.array(x)).float()).cuda(device=self.cuda_device)
        out_q = self.forward(state_moved)
        coin = random.random()
        if coin < epsilon:
            a = random.choice(self.action)
        else:
            a = out_q.data.cpu().numpy()[0].argmax()
        return a
    ...
    def forward_pure_pr(self, x, h, c):
        x_view = x.view(-1,15)
        x = F.relu(self.inv_pre.fc1(x_view))
    ...
    def choose_action_gru(self, obs, epsilon, h):
        if self.time_flag == 7:
            out, h = self.forward_gru(obs, h)
            coin = random.random()
            if coin < epsilon:
                return random.choice(self.action), h
            else:
                return out.data.cpu().numpy().argmax(), h
    ...
    def choose_action_map(self, obs, epsilon, _map):
        if self.game_name == 'babyAI':
            if self.time_flag == 6:
                out, new_map = self.forward_map(obs, _map)
                coin = random.random()
                if coin < epsilon:
                    return random.choice(self.action), new_map
                else:
                    return out.data.cpu().numpy()[0].argmax(), new_map
        ...
        if self.game_name == 'Maze':        
            if self.time_flag == 6:
                ...
                if coin < epsilon:
                    ...
                    return out.data.cpu().numpy().argmax(), new_map
    ...
    def make_batch_attention(self):
        batch_lst = []
        mini_batches = []
        for i in range(len(self.data)):
            mini_batches.append(copy.deepcopy(self.data[i]))
        for mini_batch in mini_batches:
                if self.time_flag in [4, 5]:
                    s_lst, a_lst, r_lst, s_prime_lst, done_lst, v_tar = [], [], [], [], [], []
                    for transition in mini_batch:
                    ...
                    batch = [s_lst, a_lst, s_prime_lst, v_tar]
                batch_lst.append(batch)
    ...
    def data_deal(self, pre_data):
        ...
        for data in pre_data:
            s_lst, a_lst, sp_lst, _, _, r_lst = data
            s_lst = s_lst.tolist()
            s_deal = []
            a_deal = []
            sp_deal = []
            ...
            for ind in range(len(s_lst)):
                if s_lst[ind].index(1) != sp_lst[ind].index(1):
                    s_deal.append(s_lst[ind])
                    a_deal.append(a_lst[ind])
                    sp_deal.append(sp_lst[ind])
                    r_deal.append(r_lst[ind])
            batch = [s_deal, sp_deal, a_deal, r_deal]
            batch_lst.append(batch)

        for yyy in range(len(batch_lst)):

            if yyy == 0:
                pre_deal.append(batch_lst[yyy])
                s_lst, sp_lst, a_lst, r_lst = batch_lst[yyy]
                s_buff.append(s_lst)
            else:
                s_lst, sp_lst, a_lst, r_lst = batch_lst[yyy]
                if (s_lst in s_buff) or len(s_lst)>=12:
                    number += 1
                else:
                    pre_deal.append(batch_lst[yyy])
                    s_buff.append(s_lst)
        observations = []
        for xxx in range(len(pre_deal)):
            s_lst, sp_lst, a_lst, r_lst = pre_deal[xxx]
            for s in s_lst:
                if s not in observations:
                    observations.append(s)
        observations.sort()
        return pre_deal, observations

    def make_batch_map(self):
        batch_lst = []
        mini_batches = []
        for i in range(len(self.data)):
            mini_batches.append(copy.deepcopy(self.data[i]))
    ...
    def train_pi_hci(self):
        ...
        if self.game_name == 'Maze':
            ...
            loss.backward()
            self.optimizer.step()
            if len(self.offdata)%504==0 and len(self.offdata)<=1500:
                self.train_ctf_do2()

    def train_ctf_do2(self):

        pre_data = np.array(self.offdata)
        print('offline_data:', np.shape(np.array(pre_data).tolist()))
        pre_deal, observations = self.data_deal(pre_data)
        ...
        while(1):
            ...
            print('rest observations:', observations)
            if len(observations)>num_share:
                ...
                for gg in range(num_share):
                    ...
                    for k in range(1000):
                        ...
                        a_label_lst = []
                        num_cont = 0
                        for g in range(len(pre_deal)):
                            s_lst, sp_lst, a_lst, r_lst = pre_deal[g]

                            h = torch.autograd.Variable(to_tensor(np.zeros((1, 800)))).cuda(device=self.cuda_device)
                            c = torch.autograd.Variable(to_tensor(np.zeros((1, 800)))).cuda(device=self.cuda_device)
                            ...
                            for i in range(len(s_lst)):
                                ...
                                if s_lst[i] not in current_obs and s_lst[i] not in do_buff:
                                    state_moved = torch.autograd.Variable(
                                        torch.from_numpy(np.array([[s_lst[i]]])).float()).cuda(
                                        device=self.cuda_device)
                                    _, h, c = self.forward_pure_pr(state_moved, h, c)
                                    sp_moved = torch.autograd.Variable(torch.from_numpy(np.array([[sp_lst[i]]])).float()).cuda(
                                        device=self.cuda_device)
                                    if g == num_cont and i == 0:
                                        a_pr, _, _ = self.forward_pure_pr(sp_moved, h, c)
                                    else:
                                        a_pr = torch.cat((a_pr,self.forward_pure_pr(sp_moved, h, c)[0]), 0)
                                else:
                                    state_moved = torch.autograd.Variable(
                                        torch.from_numpy(np.array([[s_lst[i]]])).float()).cuda(
                                        device=self.cuda_device)
                                    _, h2, c2 = self.forward_pure_pr(state_moved, h, c)
                                    sp_moved = torch.autograd.Variable(torch.from_numpy(np.array([[sp_lst[i]]])).float()).cuda(
                                        device=self.cuda_device)
                                    if g == num_cont and i == 0:
                                        a_pr, _, _ = self.forward_pure_pr(sp_moved, h2, c2)
                                    else:
                                        a_pr = torch.cat((a_pr, self.forward_pure_pr(sp_moved, h2, c2)[0]), 0)

                                a_label = [0, 0, 0, 0]
                                a_label[int(a_lst[i])] = 1
                                a_label_lst.append(a_label)

                        a_label_moved = torch.autograd.Variable(torch.from_numpy(np.array(a_label_lst)).float()).cuda(
                                device=self.cuda_device)
                        loss = F.smooth_l1_loss(a_pr, a_label_moved)*100
                        if k != 0:
                            if abs(loss_old - loss.data.cpu().numpy()) <= 0.0001:
                                count_loss += 1
                            else:
                                count_loss = 0
                        if count_loss >= 6:
                        ...
                        game02_file = open('ctf_loss4', 'a')
                        game02_file.write('%f %f\n' % (episode_num, loss_old))
                        game02_file.close()

                        if loss.data.cpu().numpy() < 0.1:
                    ...
                    print('number of eliminated observations:', len(do_buff))

                    print('number of rest observations:', len(observations))
                    ...
                    for k in range(1000):
                        ...
                        for g in range(len(pre_deal)):
                            ...
                            for i in range(len(s_lst)):
                                ...
                                if s_lst[i] != do_l[gg] and s_lst[i] not in do_buff:
                                    state_moved = torch.autograd.Variable(
                                        torch.from_numpy(np.array([[s_lst[i]]])).float()).cuda(
                                        device=self.cuda_device)
                                    _, h, c = self.forward_pure_pr(state_moved, h, c)
                                    sp_moved = torch.autograd.Variable(
                                        torch.from_numpy(np.array([[sp_lst[i]]])).float()).cuda(
                                        device=self.cuda_device)
                                    if g == num_cont and i == 0:
                                        a_pr, _, _ = self.forward_pure_pr(sp_moved, h, c)
                                    else:
    ...
    def train_do2(self):
        pre_data = np.array(self.offdata)
        print('pre_data:', np.shape(np.array(pre_data).tolist()))
        episode_num = 0
        pre_deal, observations = self.data_deal(pre_data)
        observations_show = []
        for sad in range(len(observations)):
            observations_show.append(observations[sad].index(1))
        print('pre_deal:', np.shape(pre_deal), 'observations:', observations_show)
        do_buff = []
        ref = 0
        do_l = copy.deepcopy(observations)
        for gg in range(len(do_l)):
            do_show = []
            for sad in range(len(do_buff)):
                do_show.append(do_buff[sad].index(1))
            print('do_buff:', do_show)
            count_loss = 0
            for k in range(1000):
                ...
                for g in range(len(pre_deal)):
                    ...
                    for i in range(len(s_lst)):
                        ...
                        if s_lst[i] != do_l[gg] and s_lst[i] not in do_buff:
                            state_moved = torch.autograd.Variable(torch.from_numpy(np.array([[s_lst[i]]])).float()).cuda(
                                device=self.cuda_device)
                            _, h, c = self.forward_pure_pr(state_moved, h, c)
                            sp_moved = torch.autograd.Variable(torch.from_numpy(np.array([[sp_lst[i]]])).float()).cuda(
                                device=self.cuda_device)
                ...
                loss = F.smooth_l1_loss(a_pr, a_label_moved) * 100
                if k != 0:
                    if loss_old - loss.data.cpu().numpy()<=0.0001:
                        count_loss += 1
                    else:
                        count_loss = 0
                if count_loss >= 15:
                ...
                loss.backward()
                self.optimizer.step()
                game02_file = open('do_loss3', 'a')
                game02_file.write('%f %f\n' % (episode_num, loss_old))
                game02_file.close()
                print(k, loss)

                if loss.data.cpu().numpy() < 0.01:
                    do_buff.append(do_l[gg])
                    break
        print('final_do_buff:', do_buff)
